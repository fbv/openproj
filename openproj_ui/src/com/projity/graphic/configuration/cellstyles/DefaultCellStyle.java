...
*/
package com.projity.graphic.configuration.cellstyles;
	...
	implements CellStyle
{
	...
	{
		GraphicNode node = (GraphicNode)_object;
		...
		if (node.getSubprojectLevel() > 0)
		{
			format.setBackground( ((node.getSubprojectLevel() % 2) == 0)
				? "MULTIPROJET0"
				: "MULTIPROJET1" );
		}
		...
		if (node.isGroup() == true)
		{
			if (node.getLevel() == 1)
			{
				format.setBackground( "TAN" );
			}
			else
			{
				format.setBackground( "LINEN" );
			}
		}
