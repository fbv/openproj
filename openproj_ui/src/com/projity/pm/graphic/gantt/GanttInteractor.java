	...
	extends GraphInteractor
{
	...
	protected void computeNodeSelection( 
		double x,
		double y )
	{
	...
	public boolean executeAction( 
		...
		double y )
	{
		if ((x == x0) || (selected == null))
		{
			return false;
		}

		if ((state == BAR_MOVE) || (state == BAR_MOVE_START) || (state == BAR_MOVE_END) || (state == PROGRESS_BAR_MOVE) ||
				(state == SPLIT))
		{
		...
		final javax.swing.undo.UndoableEditSupport editSupport = ui.getGraph().getProject().getNodeModelDataFactory()
			.getUndoController().getEditSupport();
		
		switch (state)
		{
		case BAR_MOVE:
			ScheduleService.setInterval( this, (Schedule)sourceNode.getNode().getValue(), selectedInterval.getStart() + dt,
				selectedInterval.getEnd() + dt, selectedInterval, editSupport );

			return true;

		case BAR_MOVE_START:
			ScheduleService.setInterval( this, (Schedule)sourceNode.getNode().getValue(), selectedInterval.getStart() + dt,
				selectedInterval.getEnd(), selectedInterval, editSupport );

			return true;

		case BAR_MOVE_END:
			ScheduleService.setInterval( this, (Schedule)sourceNode.getNode().getValue(), selectedInterval.getStart(),
				selectedInterval.getEnd() + dt, selectedInterval, editSupport );

			return true;

		case PROGRESS_BAR_MOVE:
			ScheduleService.setCompleted( this, (Schedule)sourceNode.getNode().getValue(), t, editSupport );
		...
		case LINK_CREATION:
			...
			try
			{
				if ((sourceNode != null) && (destinationNode != null) &&
						sourceNode.getNode().getValue() instanceof HasDependencies &&
						destinationNode.getNode().getValue() instanceof HasDependencies &&
						!ClassUtils.isObjectReadOnly( destinationNode.getNode().getValue() ))
				{
					DependencyService.getInstance()
									 .newDependency( (HasDependencies)sourceNode.getNode().getValue(),
						(HasDependencies)destinationNode.getNode().getValue(), DependencyType.FS, 0, this );
				}
			}
		...
		case SPLIT:
			ScheduleService.split( this, (Schedule)sourceNode.getNode().getValue(), t, t, editSupport );
	...
	@Override
	protected Shape getBarShadowBounds( 
		double x,
		double y )
	{
		double deltaX = x - x0;
		...
		GraphicNode node = (GraphicNode)selected;
		Rectangle2D bounds;
		double xStart = coord.toX( ((selectedIntervalNumber == 0) && (state == BAR_MOVE))
				? node.getStart()
				: selectedInterval.getStart() );
		...
		if (state == PROGRESS_BAR_MOVE)
		{
			...
			bounds = new Rectangle2D.Double(xStart,
					((GanttUI)ui).getBarY( node.getRow() ) + node.getGanttShapeOffset() +
					((node.getGanttShapeHeight() - config.getGanttProgressBarHeight()) / 2), completedX - xStart + deltaX,
					config.getGanttProgressBarHeight());
		}
		else
		{
			double xEnd = ((selectedIntervalNumber == 0) && (state == BAR_MOVE))
				? CoordinatesConverter.adaptSmallBarEndX( coord.toX( node.getStart() ), coord.toX( node.getEnd() ), node, config )
				: coord.toX( selectedInterval.getEnd() );
			double w = xEnd - xStart;

			switch (state)
			{
			case BAR_MOVE:
				bounds = new Rectangle2D.Double(xStart + deltaX,
						((GanttUI)ui).getBarY( node.getRow() ) + node.getGanttShapeOffset(), w, node.getGanttShapeHeight());

				break;

			case BAR_MOVE_START:
				bounds = new Rectangle2D.Double(xStart + deltaX,
						((GanttUI)ui).getBarY( node.getRow() ) + node.getGanttShapeOffset(), w - deltaX,
						node.getGanttShapeHeight());

				break;

			case BAR_MOVE_END:
				bounds = new Rectangle2D.Double(xStart, ((GanttUI)ui).getBarY( node.getRow() ) + node.getGanttShapeOffset(),
						w + deltaX, node.getGanttShapeHeight());
	...
	protected Rectangle2D getLinkSelectionShadowBounds( 
		GraphicNode node )
	{
		CoordinatesConverter coord = getCoord();
		...
		Rectangle2D selectionRectangle = new Rectangle2D.Double(xStart,
				((GanttUI)ui).getBarY( node.getRow() ) + node.getGanttShapeOffset(), xEnd - xStart, node.getGanttShapeHeight());
	...
	protected boolean isReadOnlyPercentComplete()
	{
		...
		Object impl = ((GraphicNode)selected).getNode().getValue();
	...
	protected void setLinkOrigin()
	{
		GraphicNode node = (GraphicNode)selected;
		CoordinatesConverter coord = getCoord();
		double xStart = coord.toX( (selectedIntervalNumber == 0)
				? node.getStart()
				: selectedInterval.getStart() );
		double xEnd = (selectedIntervalNumber == 0)
			? CoordinatesConverter.adaptSmallBarEndX( coord.toX( node.getStart() ), coord.toX( node.getEnd() ), node, config )
			: coord.toX( selectedInterval.getEnd() );
	...
	protected boolean switchOnLinkCreation( 
		double x,
		double y )
	{
		if (state == PROGRESS_BAR_MOVE)
		...
		GraphicNode node = (GraphicNode)selected;
		Object impl = node.getNode().getValue();

		return impl instanceof HasDependencies && ((((int)y) / ((Gantt)getGraph()).getRowHeight()) != node.getRow());
	}
		...
		implements IntervalConsumer
	{
			...
			ScheduleInterval interval )
		{
			if (consumed)
			{
				return; //Consumer need to consume all the intervals
			}

			if (ClassUtils.isObjectReadOnly( ((GraphicNode)selected).getNode().getValue() )) // pre
			{
				return;
			}
			...
			if (coord != null)
			{
				...
				if ((completedT >= interval.getStart()) && (completedT <= interval.getEnd()))
				{
					...
					{
						completedInterval = coord.adaptSmallBarTimeInterval( completedInterval, node, config );
					}
				}
			...
			else if ((completedInterval != null) && (t >= (completedInterval.getEnd() - completedDeltaT0)) &&
					(t <= (completedInterval.getEnd() + completedDeltaT1)) && (selectedZone != null) &&
					(selectedZone.getZoneId() == GanttUI.PROGRESS_BAR_ZONE_ID))
			{
				if (selectedIsNonSummaryNode() && !isReadOnlyPercentComplete())
				{
					state = PROGRESS_BAR_MOVE;
				}
			}
		...
		public NodeSelectionIntervalConsumer init( 
			double x,
			GraphicNode node )
		{
