	...
	extends GraphUI
{
		...
		double y )
	{
		...
		if ((row < 0) || (row >= graph.getModel().getCache().getSize()))
		{
			return null;
		}
		...
		if (node.contains( t, deltat, deltat, coord ) == null)
		{
			return null;
		}
		...
		if ((y >= ((y0 + (h / 2)) - (progessH / 2))) && (y < (y0 + (h / 2) + (progessH / 2))))
		{
