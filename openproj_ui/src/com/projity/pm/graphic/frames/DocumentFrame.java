		...
		ObjectSelectionListener
{
		...
		String id )
	{
		...
		filterToolBarManager = (Environment.isNewLook() && !Environment.isNoPodServer())
			? FilterToolBarManager.create( menuManager )
			: graphicManager.getFilterToolBarManager();
	...
	public void activateBottomView( 
		BaseView view,
		String viewName )
	{
	...
	public Node addNodeForImpl( 
		Object _impl )
	{
		return addNodeForImpl( _impl, NodeModel.NORMAL );
	}
		...
		int _eventType )
	{
		...
		{
			return spreadSheet.addNodeForImpl( _impl, _eventType );
		}
	}
		...
		return new Closure()
			{
					...
					Object arg )
				{
					...
					if ((hiddenFilter != null) && hiddenFilter instanceof ResourceInTeamFilter)
					{
						...
						rf.setFilterTeam( GlobalPreferences.getInstance().isShowProjectResourcesOnly() );
	...
	public void doSubstituteResource()
	{
		...
		final ResourceSubstitutionDialog dlg = ResourceSubstitutionDialog.getInstance( getGraphicManager().getFrame(), null,
				hasAtLeastOneTaskSelected(), getProject() );
			...
			Command command = new Command(Messages.getString( "ResourceSubstitutionDialogBox.title" ), getProject())
				{
						...
						Object arg0 )
					{
						...
						AssignmentService.getInstance()
										 .replaceResourceOnTask( (Task)arg0, dlg.getForm().getFromResource(),
							dlg.getForm().getToResource(), dlg.getForm().isIgnoreInProgress(), startingTime );
					}
				};
	...
	private void forTasksDo( 
		Closure closure,
		boolean all )
	{
		DataUtils.forAllDo( closure, all, project.getTaskOutlineIterator(), getSelectedNodes( true ), Task.class );
	}
	...
	private WorkspaceSetting getWorkspaceFor( 
		BaseView view )
	{
	...
	public void groupDirtyChanged( 
		final ProjectEvent e )
	{
	...
	public boolean isCurrentRowInMainProject()
	{
		...
		while (i.hasPrevious())
		{
			Object o = ((Node)i.previous()).getValue();
...
//			}
		...
		else if ((objectEvent.getObject() instanceof GlobalPreferences == true)
			  && (objectEvent.isUpdate() == true))
		{
			//if (objectEvent.getField() == Configuration.getFieldFromId("Field.showProjectResourcesOnly")) {
			for (ResourceInTeamFilter filter : resourcesInTeamFilters)
			{
				filter.setFilterTeam( GlobalPreferences.getInstance().isShowProjectResourcesOnly() );
			}
	...
	public void setComboBoxesViewName( 
		String view )
	{
	...
	{
		return (activeTopView == null)
		? false
		: activeTopView.canScrollToTask();
	}
	...
	public NodeModelCache createCache( 
		...
		String viewName )
	{
		return NodeModelCacheFactory.getInstance().createFilteredCache( getReferenceCache( task ), viewName, null );
	}
	...
	public ChartView getChartView()
	{
		if (chartView == null)
		{
			chartView = new ChartView(this, false, graphicManager.getMenuManager(), mainView.getSynchronizer(),
					addTransformerInitializationClosure());
	...
	public ChartView getHistogramView()
	{
		...
		{
			histogramView = new ChartView(this, true, graphicManager.getMenuManager(), mainView.getSynchronizer(),
					addTransformerInitializationClosure());
	...
	public TreeView getRBSView()
	{
		if (rbsView == null)
		{
			...
			rbsView.init( getResourceNodeModelCache(), getResourceModel(), ACTION_RBS, addTransformerInitializationClosure() );
	...
	public ReferenceNodeModelCache getReferenceCache( 
		boolean task )
	{
		ReferenceNodeModelCache cache = (task)
			? getTaskNodeModelCache()
	...
	public BaseView getReportView()
	{
		try
		{
			if (reportView == null)
			{
				...
				reportView = (BaseView)clazz.getConstructor( new Class[]
						{
							DocumentFrame.class
				...
				clazz.getMethod( "init", new Class[]
					{
						CoordinatesConverter.class
					} ).invoke( reportView, new Object[]
					{
						coord
					} );
				...
				if (reportView != null)
				{
					restoreWorkspaceFor( reportView );
				}
			}
		}
		catch (Exception e)
	...
	public UsageDetailView getResourceUsageDetailView()
	{
		...
		{
			resourceUsageDetailView = new UsageDetailView(this, graphicManager.getMenuManager(), mainView.getSynchronizer());
			resourceUsageDetailView.init( getResourceNodeModelCache(), false, coord, false, ACTION_RESOURCE_USAGE_DETAIL,
				addTransformerInitializationClosure() );
	...
	public UsageDetailView getResourceUsageView()
	{
		...
		{
			resourceUsageView = new UsageDetailView(this, graphicManager.getMenuManager(), mainView.getSynchronizer());
			resourceUsageView.init( getResourceNodeModelCache(), false, coord, true, ACTION_RESOURCE_USAGE,
				addTransformerInitializationClosure() );
	...
	public ResourceView getResourceView()
	{
		...
		{
			resourceView = new ResourceView(getResourceNodeModelCache(), getResourceModel(), project.getResourcePool(),
					!Environment.isOpenProj() && project.isReadOnly(), graphicManager.isEditingMasterProject());
	...
	public UsageDetailView getTaskUsageDetailView()
	{
		...
		{
			taskUsageDetailView = new UsageDetailView(this, graphicManager.getMenuManager(), mainView.getSynchronizer());
			taskUsageDetailView.init( getTaskNodeModelCache(), true, coord, false, ACTION_TASK_USAGE_DETAIL,
				addTransformerInitializationClosure() );
	...
	public UsageDetailView getTaskUsageView()
	{
		...
		{
			taskUsageView = new UsageDetailView(this, graphicManager.getMenuManager(), mainView.getSynchronizer());
			taskUsageView.init( getTaskNodeModelCache(), true, coord, true, ACTION_TASK_USAGE,
				addTransformerInitializationClosure() );
	...
	private WorkspaceSetting restoreWorkspaceFor( 
		BaseView view )
	{
	...
	private void saveViewWorkspace( 
		Workspace ws,
		String name,
		BaseView view )
	{
	...
	boolean doBaselineDialog( 
		boolean save )
	{
		finishAnyOperations();
		getGraphicManager().lockProject(); // pre-lock project for timeout issue

		BaselineDialog baselineDialog = BaselineDialog.getInstance( getGraphicManager(), null, save, hasAtLeastOneTaskSelected() );
		...
		Integer baselineNumber = new Integer(baselineDialog.getForm().getBaselineNumber());
		boolean entireProject = baselineDialog.getForm().isEntireProject();
		List selection = entireProject
			? null
			: getSelectedImpls( true );

		if (save)
		{
			getProject().saveCurrentToSnapshot( baselineNumber, entireProject, selection, true );
		}
		else
		{
			getProject().clearSnapshot( baselineNumber, entireProject, selection, true );
		}
	...
	//	}
	void doChangeWorkingTimeDialog( 
		boolean restrict )
	{
		finishAnyOperations();
		...
		if (rowObject instanceof HasCalendar)
		{
			...
			{
				documentCalendars = ((ResourceImpl)rowObject).getResourcePool().extractCalendars();
			}
		}
		...
		ChangeWorkingTimeDialogBox dlg = ChangeWorkingTimeDialogBox.getInstance( getGraphicManager().getFrame(), project, wc,
				documentCalendars, restrict, this.getUndoController() );
		dlg.doModal();
	}
	...
	void doDelegateTasksDialog()
	{
		...
		DelegateTaskDialog dlg = DelegateTaskDialog.getInstance( getGraphicManager().getFrame(), nodes );
	...
	void doUpdateProjectDialog()
	{
		...
		UpdateProjectDialogBox dlg = UpdateProjectDialogBox.getInstance( getGraphicManager().getFrame(), null,
				hasAtLeastOneTaskSelected() );

		if (dlg.doModal())
		{
			UpdateProjectCommand cmd = new UpdateProjectCommand(project, dlg.getForm().getActiveDate().getTime(),
	...
	void doUpdateTasksDialog()
	{
		...
		UpdateTaskDialog dlg = UpdateTaskDialog.getInstance( getGraphicManager().getFrame(), nodes );
		dlg.setLocationRelativeTo( null );
	...
	List<Node<?>> getSelectedNodes( 
		final boolean _excludeReadOnly )
	{
		CommonSpreadSheet spreadSheet = getTopSpreadSheet();
		...
		List<Node<?>> nodes = spreadSheet.getSelectedNodes();
			...
			SwingUtilities.invokeLater( new Runnable()
				{
					public void run()
					{
						if (true || (GraphicManager.getLastWorkspace() == null))
						{ // if there was a workspace, it will be restored
		...
		implements WorkspaceSetting
	{
		...
		public final void setBottomViewName( 
		...
		public final void setTopViewName( 
			String topViewName )
		{
