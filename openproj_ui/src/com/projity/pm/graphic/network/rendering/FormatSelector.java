...
public class FormatSelector {
	...
	private class FormatSelectionFunctor implements Closure<BarFormat> {
		void initialize() {
			selectedFormats.clear();
		}
		public void execute(final BarFormat _barFormat) {			
			if (!_barFormat.isMain()) return;
			selectedFormats.add( _barFormat );
		}
	}
	
	void selectFormats(GraphicNode gnode){
		Node node=gnode.getNode();
		Object ganttableObject =node.getValue();
