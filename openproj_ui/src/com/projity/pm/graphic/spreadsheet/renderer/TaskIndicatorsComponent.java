...
public class TaskIndicatorsComponent extends IndicatorsComponent{
	...
	public void setIndicators(Object value,JComponent label,StringBuffer text,boolean isSelected, boolean hasFocus){
		...
		if (indicators instanceof Task) {
			project = ((Task)indicators).getProject();
			actualsProtected = ((Task)indicators).isActualsProtected();
		...
		if (indicators instanceof Task && indicators.isParentWithAssignments()) {
			...
			text.append(Messages.getString("TaskIndicatorsComponent.ThisParentTaskHasResources") + ((Task)indicators).getResourceNames()+"<br>"); //$NON-NLS-1$ //$NON-NLS-2$
		}
