		...
		Searchable
{
		...
		int eventType )
	{
		...
		if (row == -1)
		{ 
			// fix for bug when inserting subproject and no selection
	...
	public void addSelectionNodeListener( 
		SelectionNodeListener l )
	{
		...
		boolean forwards )
	{
		...
		if (editOnSelect && !isEditing())
		{
			...
			//TODO prevent setValueAt from being called because of this on selection
		}
		else
		{
			//
	...
	protected void configureScrollPaneHeaders( 
			...
			else if (comp instanceof ChangeAwareTextField)
			{
				...
				else if (e instanceof MouseEvent)
				{
					if (nameCell)
					{
						...
						{ //because if it's outside there's no caret update
							text.requestFocus();
							text.selectAll();
						}
					}
		...
		SearchContext context )
	{
		...
		if (ctx.getRow() != -1)
		{ // after the first search, need to move ahead or back
		...
		while (ctx.isForward()
				? i.hasNext()
					: i.hasPrevious())
		{
			gnode = (GraphicNode)(ctx.isForward()
				? i.next()
				: i.previous());
		...
		Node currentNode )
	{
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2)
		{
			if (listeners[ i ] == SelectionNodeListener.class)
			{
				if (e == null)
				{
					e = new SelectionNodeEvent(source, SelectionNodeEvent.SELECTION_CHANGED, nodes, currentNode,
							getSpreadSheetCategory());
				}
	...
	public NodeModelCache getCache()
	{
		...
		if ((model == null) || !(model instanceof CommonSpreadSheetModel))
		{
			return null;
		}
	...
	public final SpreadSheetFieldArray getFieldArrayWithWidths( 
	...
	public ArrayList getSelectableFields()
	{
		...
		if (fields.size() > 0)
		{
			fields.remove( 0 ); //ID not selectable
		}

		return fields;
	}

	public List getSelectedCuttableRows( 
		List nodes )
	{
		CollectionUtils.filter( nodes,
			new Predicate()
			{
				@Override
	...
	{
		if (getRowHeader().getSelectedColumns().length > 0)
		{
			return null;
		}
	...
	public ArrayList getSelectedNodesImpl()
	{
		...
		for (int i = 0; i < rows.length; i++)
		{
			nodes.add( model.getNode( rows[ i ] ).getNode().getValue() );
		}
	...
	public EventListener[] getSelectionNodeListeners( 
		Class listenerType )
	{
	...
	public void graphicNodesCompositeEvent( 
		CompositeCacheEvent compositeEvent )
	{
	...
	protected void initRowHeader( 
		CommonSpreadSheetModel spreadSheetModel )
	{
	...
	public boolean isNodeCuttable( 
		Node node )
	{
		return true;
	}

	public boolean isNodeDeletable( 
		Node node )
	{
		return true;
	}

	public void removeSelectionNodeListener( 
		SelectionNodeListener l )
	{
...
//	}
	...
	public final void setCanModifyColumns( 
		boolean canModifyColumns )
	{
		this.canModifyColumns = canModifyColumns;
	}

	public final void setCanSelectFieldArray( 
		boolean canSelectFieldArray )
	{
	 ...
	 */
	public void setEditOnSelect( 
		boolean editOnSelect )
	{
	...
	public final void setFieldArrayWithWidths( 
		SpreadSheetFieldArray fieldArray )
	{
	 ...
	 */
	public void setSpreadSheetCategory( 
		...
		int arg2 )
	{
		...
		{ // because setValue has no exceptions, I package it in a runtime one
		...
		implements WorkspaceSetting
	{
		...
		public final void setEditingColumn( 
			int editingColumn )
		{
			this.editingColumn = editingColumn;
		}

		public final void setEditingRow( 
			int editingRow )
		{
			this.editingRow = editingRow;
		}

		public final void setFieldArrayName( 
			String fieldArrayName )
		{
			this.fieldArrayName = fieldArrayName;
		}

		public final void setLastEditingRow( 
			int lastEditingRow )
		{
			this.lastEditingRow = lastEditingRow;
		}

		public final void setSelectedColumns( 
			int[] selectedColumns )
		{
			this.selectedColumns = selectedColumns;
		}

		public final void setSelectedRows( 
			int[] selectedRows )
		{
