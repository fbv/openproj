	...
	@Override
		...
		private boolean transformSubprojectBranches(Node parent,NodeModelDataFactory dataFactory,Predicate p){
			if (dataFactory instanceof Project &&
					parent.getValue() instanceof SubProj
//					&&!((Project)dataFactory).getSubprojectHandler().canInsertProject( ((SubProj)parent.getValue()).getSubprojectUniqueId() )
	        	    	...
	        	    	for (Iterator i=nodes.iterator();i.hasNext();) {
	        				...
	        				transformSubprojectBranches(node,model.getDataFactory(),new Predicate(){
								public boolean evaluate(Object arg0) {
									...
									Task task = Model.getInstance().createTask();
									Task source = ((Task)parent.getValue());
									source.cloneTo( task );
