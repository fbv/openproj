...
//		Node node=getNodeInRow(row/getRowMultiple());
		...
		getFieldContext().setInterval((HasStartAndEnd)timeIntervals.get(col-1));
		 ...
		 return NodeModelUtil.getValue( getFieldInRow(row), node, getCache().getWalkersModel(), fieldContext); //TODO
	}
	...
	public boolean isCellEditable(int row, int col) {
		...
		return ! NodeModelUtil.isReadOnly( getFieldInRow(row), node, getCache().getWalkersModel(),fieldContext);
	}
