...
{
	private ProjectInformationDialog( 
		final Frame _owner,
		final Project _project )
	{
		super( _owner, Messages.getString( "ProjectInformationDialog.ProjectInformation" ) );
	...
	private JComponent createGeneralPanel()
	{
		...
		builder.nextLine( 2 );
		map.append( builder, "Field.manager" );
		builder.nextLine( 2 );

		map.appendSometimesReadOnly( builder, "Field.startDate" );
		map.append( builder, "Field.currentDate" );
		builder.nextLine( 2 );
		map.appendSometimesReadOnly( builder, "Field.finishDate" );
		map.append( builder, "Field.statusDate" ); 

		builder.nextLine( 2 );
		map.append( builder, "Field.forward" ); 
//		builder.nextColumn( 2 );
		map.append( builder, "Field.baseCalendar" ); 
		builder.nextLine( 2 );
	...
	{
		FormLayout layout = new FormLayout("p, 3dlu, 300dlu", "p" );
		DefaultFormBuilder builder = new DefaultFormBuilder( layout );
		map.append( builder, "Field.name" );

		return builder.getPanel();
	}

	/* (non-Javadoc)
	 * @see com.projity.dialog.FieldDialog#createMap()
	 */
	@Override
	protected FieldComponentMap createMap() 
	{
		FieldComponentMap map = new FieldComponentMap( myProject, myProject.getNodeModelDataFactory() );
		myMaps.add(map);
		return map;
	}
	...
	public void documentSelected( 
		DocumentSelectedEvent evt )
	{
		setObject( evt.getCurrent().getProject() );
		updateAll();
	}

	public static ProjectInformationDialog getInstance( 
		Frame owner,
		Project project )
	{
	...
	/* (non-Javadoc)
	 * @see com.projity.dialog.FieldDialog#setObject(java.lang.Object)
	 */
	@Override
