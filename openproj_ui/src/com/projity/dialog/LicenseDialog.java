	...
	extends AbstractDialog 
{
	...
	public static boolean showDialog(Frame owner, boolean force) 
	{
		...
		{
			System.setProperty( "openproj.validation", Preferences.userNodeForPackage( LicenseDialog.class )
				.get( "licenseValidationDate", "0" ) );
		}
		...
		final int height ) 
	{
		...
		try 
		{
			pane = new JEditorPane( url ) 
			{
				...
				{
					//TODO there are issues with the size, so i just make it correct here
	...
	protected void initComponents() 
	{
		...
		if (Environment.isOpenProj() == true) 
		{
			license = createEditorPane( getClass().getClassLoader().getResource( "license/index.html" ), 7500 );
			thirdParty = createEditorPane( getClass().getClassLoader().getResource( "license/third-party/index.html" ), 1200 );
		} 
		else 
		{
			try 
			{
				thirdParty = createEditorPane( new URL( "http://projity.com/license/third-party/index.html" ), 1200 );
			} 
...
//		thirdParty = createEditorPane("http://www.projity.com/license/third-party/index.html",800); //$NON-NLS-1$
	...
	public void onOk() 
	{
		validated = true;
		if (resetData)
		{//About dialog shouldn't reset data
