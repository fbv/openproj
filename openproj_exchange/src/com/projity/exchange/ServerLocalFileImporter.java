	...
	extends ServerFileImporter
{
	...
	public Job getImportFileJob()
	{
		...
		final Job job = new Job( importer.getJobQueue(), "importFile", Messages.getString( "LocalFileImporter.Importing" ), true );
		job.addRunnable( 
		...
		{ 
				...
				throws Exception
			{
				...
				{
					if (job.okCancel( Messages.getString( "Message.ServerUnreacheableReadOnlyProject" ), true ) == false)
					{
						setProgress( 1.0f );
...
//    	        project=serializer.deserializeProject(projectData,false,true,resourceMap);
				...
				serializer.setStuffForPODDeserialization( form.getExistingProject(), myLocalResourceMap );
		...
		throws Exception
	{
		 ...
		 && (form.isLocal() == false))
		{
			if (Environment.isNoPodServer() == true)
			{
				...
				final Project existingProject = form.getExistingProject();
	...
	protected void renumberProject()
		throws Exception
	{
		...
		throws Exception
	{
		...
		//map the existing project resourse impls for later use
		myLocalResourceMap = new HashMap<Long,com.projity.pm.resource.Resource>();
		...
		if (existingProject.isForceNonIncremental())
		{
		...
		List<com.projity.pm.resource.Resource> existingResources = (List<com.projity.pm.resource.Resource>)existingProject
			.getResourcePool().getResourceList();
		...
		for (com.projity.pm.resource.Resource resource : existingResources)
		{
			...
			if (r != null)
			{
				...
				myLocalResourceMap.put( (Long)r.getUniqueId(), resource );
			}
		}
	...
	Map<Long,com.projity.pm.resource.Resource> myLocalResourceMap;
}
