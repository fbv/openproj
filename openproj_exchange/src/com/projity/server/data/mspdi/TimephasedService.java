...
public class TimephasedService {
	...
	private void doQuery(Assignment assignment, ObjectFactory factory, TimephasedConsumer consumer,TimeDistributedConstants fieldType, int type, long id) {
		...
		if (fieldType == TimeDistributedConstants.ACTUAL_WORK)
			end = assignment.getStop();
		else if (fieldType == TimeDistributedConstants.REMAINING_WORK)
	...
	public void consumeTimephased(Schedule schedule,TimephasedConsumer consumer,Object factory) throws JAXBException{
		...
		if ( assignment.getPercentComplete() > 0) {
			doQuery(assignment,mspdiTimephasedFactory, consumer,TimeDistributedConstants.ACTUAL_WORK, TimeDistributedTypeMapper.ASSIGNMENT_ACTUAL_WORK, id++);
		}
		doQuery(assignment,mspdiTimephasedFactory, consumer,TimeDistributedConstants.REMAINING_WORK, TimeDistributedTypeMapper.ASSIGNMENT_REMAINING_WORK, id++);
		...
		for (int i = 0; i < fields.length; i++) {
			...
			doQuery(baselineAssignment,mspdiTimephasedFactory, consumer,TimeDistributedConstants.WORK, mapType, id++);
		}
	}
