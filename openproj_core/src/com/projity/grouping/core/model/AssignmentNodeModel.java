...
{
	public AssignmentNodeModel( 
		Document document,
		boolean containsLeftObjects )
	{
		super();
	...
	/*ArrayList list,*/ MutableNodeHierarchy hierarchy,
		NodeModelDataFactory dataFactory,
		Document document,
		boolean containsLeftObjects )
	{
		super(
		/*list,*/ hierarchy, dataFactory);
		this.containsLeftObjects = containsLeftObjects;
		setDocument( document );
	}
		...
		Iterator i )
	{
		...
		{ // go thru tasks or resources
			...
			if (!(parent.getValue() instanceof HasAssignments))
			{
				continue; //TODO currently getting voidNodeImpl's.  This should go away when fixed
			}

			HasAssignments hasAssignments = (HasAssignments)parent.getValue();
		...
		ObjectEvent objectEvent )
	{
		if (objectEvent.getObject() instanceof Assignment)
		{
			...
			{ //TODO check if it's correct
				...
				{
					Object parentObject = containsLeftObjects
						? assignment.getLeft()
						: assignment.getRight();
					Node parent = search( parentObject );

					if (parent != null)
					{ // the new assignment has to be added

						Node child = null;

						if ((objectEvent.getInfo() != null) && myDataFactory instanceof Project)
						{
						...
						{
							if (!(((Node)e.nextElement()).getValue() instanceof Assignment))
							{
								break;
							}
						}
						...
						if (((objectEvent.getInfo() == null) ||
								...
								myDataFactory instanceof Project)
						{
		...
		int actionType )
	{
		...
		{
			myHierarchy.visitLeaves( (Node)i.next(),
				new Closure()
				{
				@Override
					public void execute( 
						Object o )
					{
						...
						{
							Assignment assignment = (Assignment)node.getValue();
							Node parent = (Node)node.getParent();

							if (parent.getValue() instanceof Task)
							{
								Task task = (Task)parent.getValue();
		...
		for (Iterator i = freeAssignments.iterator(); i.hasNext();)
		{
			...
			System.out.println( "restoring assignment: " + node.getValue() );
			AssignmentService.getInstance().connect( node, this, isUndo( actionType ) );
		}
	}

	public void setContainsLeftObjects( 
		boolean containsLeftObjects )
	{
		this.containsLeftObjects = containsLeftObjects;
	}
