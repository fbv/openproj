	...
	implements NodeHierarchy
{
	...
	{
		hierarchyListenerList.add( HierarchyListener.class, _listener );
	}
	...
	public EventListener[] getHierarchyListeners( 
		Class listenerType )
	{
		...
		visitAll( new Closure()
			{
					...
					Object o )
				{
					...
					if (node.hasNumber())
					{
						HasKey impl = (HasKey)node.getValue();
		...
		visitAll( new Closure()
			{
				public void execute( 
					Object node )
				{
					if ((filter != null) && !filter.evaluate( ((Node)node).getValue() ))
					{
						return;
					}
	...
	public void visitAll( 
		Node parent,
		boolean skipLazyParents,
		Closure<Node> visitor )
	{
		...
		Closure<Node> visitor )
	{
		...
		if (!first && skipLazyParents && (parent != null) && parent.getValue() instanceof LazyParent)
		{
			return;
		}

		final Collection<Node> children = getChildren( parent );

		if (children != null)
		{
			final Iterator<Node> itor = children.iterator();
	...
	@Override
	public void visitAllLevelOrder( 
		Node parent,
		boolean skipLazyParents,
		Closure visitor )
	{
		...
		Closure<Node> visitor )
	{
		...
		if (children != null)
		{
			...
			while (itor.hasNext() == true)
			{
				visitor.execute( itor.next() );
			}

			final Iterator<Node> itor2 = children.iterator();

			while (itor2.hasNext() == true)
			{
				visitAllLevelOrder( false, itor2.next(), skipLazyParents, visitor );
			}
		}
	}
		...
		boolean doChildren )
	{
		...
		if (doChildren)
		{ // if haven't visited children yet
		...
		while (i.hasNext())
		{ // get next element after this one.  If it is the last then try its parent
	...
	{
		public ShallowPreorderInterator( 
			TreeNode rootNode,
			int maxLevel,
			boolean returnRoot )
		{
		...
		public TreeNode next()
		{
			...
			Enumeration children = (level == maxLevel)
				? null
				: node.children();
