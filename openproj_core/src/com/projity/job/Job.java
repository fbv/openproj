...
//	static Log log = LogFactory.getLog(Job.class);
	...
	protected Closure myCancelMonitorClosure=null;
...
//		log.info("Job "+getName()+": "+s);
	...
	public void setCancelMonitorClosure(
		Closure<Object> _cancelMonitorClosure )
	{
		myCancelMonitorClosure = _cancelMonitorClosure;
	}
	...
	public void run(boolean sync){
		try{
			...
			if (showProgess){
				...
				if (progressMonitor!=null&&myCancelMonitorClosure!=null&&monitorChecker==null){
						...
						public void run() {
							...
							while(true){
								if (progressMonitor.isCanceled()){
									myCancelMonitorClosure.execute(null);
									break;
			...
			SwingUtilities.invokeLater(new Runnable()
			{
		    	...
		    	public void run()
				{
					...
					finally
					{
		    			...
		    			if ((isCustomCriticalSection() == false)
							&&(runnable == lastRunnable
							|| runnable.getException() != null)) 
						{
							jobQueue.endCriticalSection( Job.this );
						}
		    		}
		        }
		    });
