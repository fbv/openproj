...
public class UniqueIdPool
{
		...
		throws UniqueIdException
	{
		...
		if (serverIntervals.isEmpty() == true)
		{
			//if (onlyGlobal){
			try
			{
				makeServerReservationSync( idCount, _session );
			}
...
//			else makeServerReservationAsync(idCount);
		...
		if (size < getMinIdCount())
		{
			makeServerReservationAsync( idCount - size, _session );
		}
...
//		return r;
	...
	protected int getIdCount()
	{
		...
		if ((lastIdReservation != -1) && ((t - lastIdReservation) < 10000))
		{
	...
	protected void makeServerReservation( 
		final int _count,
		final Session _session )
		throws Exception
	{
		...
		MutableInterval interval = (MutableInterval)SessionFactory.call( _session, "bookUIDInterval", 
			new Class[]
			{
				int.class
			}, 
		...
		synchronized (serverIntervals)
		{
			serverIntervals.add( interval );
		}

		System.out.println( "ID reservation, new pool: " + dump() );
	}

	protected void makeServerReservationAsync( 
		final int _count,
		final Session _session )
	{
		Thread idBookingThread = new Thread()
		{
			...
			public void run()
			{
				synchronized (UniqueIdPool.this)
				{
					if (reservationSem > 0)
					{
						return;
				...
				try
				{
					makeServerReservation( _count, _session );
				}
				catch (final Exception _e)
				{
					System.out.println( "Id cannot be retrieved: " + _e );
				}
				finally
				{
					synchronized (UniqueIdPool.this)
	...
	protected void makeServerReservationSync( 
		final int _count,
		final Session _session )
		throws Exception
	{
