	...
	extends AssignmentFieldOvertimeFunctor
{
	 ...
	 * @param _interval The SelectFrom from the algorithm
	 */
	@Override
	public void execute( 
		final HasStartAndEnd _interval )
	{
		...
		if ((bucket != null) 
		 && (bucket.getUnits() != 0))
		{ 
			// neither regular or overtime if contour has 0 units
			double bucketDuration = myWorkCalendar.compare( _interval.getEnd(), _interval.getStart(), false );
			...
			myRegularValue += (bucket.getEffectiveUnits( myAssignment.getUnits() ) * bucketDuration);
			myOvertimeValue += (myOvertimeUnits * bucketDuration);
			myValue = myRegularValue + myOvertimeValue;
...
//			System.out.println("interval " + new java.util.Date(interval.getStart()) + " " + new java.util.Date(interval.getEnd()) +  " bucket " + com.projity.datatype.DurationFormat.format((long) bucketDuration) + " units " + bucket.getEffectiveUnits(assignment.getUnits()) + " perso " + (bucket instanceof PersonalContourBucket));
	...
	public static WorkFunctor getInstance( 
		Assignment assignment,
		WorkCalendar workCalendar,
		ContourBucketIntervalGenerator contourBucketIntervalGenerator,
		double overtimeUnits )
