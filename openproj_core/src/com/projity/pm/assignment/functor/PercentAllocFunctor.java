	...
	extends AssignmentFieldFunctor
{
	...
	{
		final AbstractContourBucket bucket = (AbstractContourBucket)myContourBucketIntervalGenerator.current();

		if (bucket != null)
		{
			long bitOfWork = myWorkCalendar.compare( _interval.getEnd(), _interval.getStart(), false );
			work += bitOfWork;
			myValue += (bucket.getEffectiveUnits( myAssignment.getUnits() ) * bitOfWork);
		}
	}

	public static PercentAllocFunctor getInstance( 
		com.projity.pm.assignment.Assignment assignment,
		com.projity.pm.calendar.WorkCalendar workCalendar,
	...
	public double getValue()
	{
		...
		return Math.min( myValue / work, maximumUnits );
	}
