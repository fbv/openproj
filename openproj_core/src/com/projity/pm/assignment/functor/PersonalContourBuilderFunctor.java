 ...
 */
public class PersonalContourBuilderFunctor
	extends AssignmentFieldFunctor
{
	private PersonalContourBuilderFunctor( 
		Assignment assignment,
		WorkCalendar workCalendar,
		ContourBucketIntervalGenerator contourBucketIntervalGenerator,
		CollectionIntervalGenerator replacementGenerator,
	...
	{
		if (_interval.getStart() == 0) // ignore degenerate range 
		{
			return;
		}
		...
		if (replacementGenerator.isCurrentActive() && (replacementGenerator.getStart() == _interval.getStart()))
		{
			intervalDuration = myWorkCalendar.compare( replacementGenerator.getEnd(), replacementGenerator.getStart(), false ); // get duration of new region

			// if inserting during a non-working time, need to adjust assignment calendar
			if (intervalDuration == 0)
			{
				myAssignment.addCalendarTime( _interval.getStart(), _interval.getEnd() );
			}

			//need to shift start to make room for new ones 
			if (_interval.getStart() < myAssignment.getStart())
			{
				myAssignment.setStart( _interval.getStart() );
			}
			...
			activeDate = replacementGenerator.getEnd(); // ignore everything in the future until active date
		}
		else if (_interval.getStart() >= activeDate)
		{ // use contour bucket
			intervalDuration = myWorkCalendar.compare( _interval.getEnd(), _interval.getStart(), false );
			...
			if (myContourBucketIntervalGenerator.getBucket() == null)
			{ // if not active, then insert dead time
				bucket = PersonalContourBucket.getInstance( intervalDuration, 0 ); // make a new non-workingbucket
			}
			else
			{
				bucket = myContourBucketIntervalGenerator.getBucket();
		...
		if ((previous != null) && (previous.getUnits() == bucket.getUnits()))
		{
			...
			bucket = PersonalContourBucket.getInstance( bucket.getBucketDuration( assignmentWork ) +
					previous.getBucketDuration( assignmentWork ), previous.getUnits() );
		}
	...
	public static PersonalContourBuilderFunctor getInstance( 
		Assignment assignment,
		WorkCalendar workCalendar,
		com.projity.pm.assignment.contour.ContourBucketIntervalGenerator contourBucketIntervalGenerator,
		CollectionIntervalGenerator replacementGenerator,
		Collection collection )
	{
		return new PersonalContourBuilderFunctor(assignment, workCalendar, contourBucketIntervalGenerator, replacementGenerator,
			collection);
	}
