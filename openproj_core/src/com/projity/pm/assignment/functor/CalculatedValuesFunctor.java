 ...
 *
 */
public class CalculatedValuesFunctor
	extends AssignmentFieldFunctor
{
	/**
	 *
	 */
	private CalculatedValuesFunctor( 
		AssignmentFieldFunctor child,
		CalculatedValues calculatedValues,
		TimeIteratorGenerator generator )
	{
		super(child.myAssignment, child.myWorkCalendar, child.myContourBucketIntervalGenerator);
		this.child = child;
		...
		final HasStartAndEnd _interval )
	{
		...
		{
			child.reset();
			child.execute( _interval );
		}
		
		calculatedValues.set( (generator == null)
			? 0
			: generator.getIndex(), _interval.getStart(), _interval.getEnd(), ((DoubleValue)child).getValue(),
			child.getWorkCalendar() );
	}

	public static CalculatedValuesFunctor getInstance( 
		AssignmentFieldFunctor child,
		CalculatedValues calculatedValues,
		TimeIteratorGenerator generator )
	{
