...
{
	private DependencyFormat( 
		AssociationFormatParameters parameters )
	{
		super(parameters);
	}
		...
		FieldPosition fieldPos )
	{
		...
		Task task = (Task)((parameters.isLeftAssociation())
	...
	public static DependencyFormat getInstance( 
		AssociationFormatParameters parameters )
	{
	...
	protected Object createNewObject( 
		final boolean _left )
	{
		//TODO this should not search only in current
		return ((Task)parameters.getThisObject()).getProject().newTaskInstance(); 
	}
	 ...
	 */
	@Override
	protected Collection getContainer( 
		final boolean _left )
	{
		return ((Task)parameters.getThisObject()).getProject().getTasks().collection();
	}
	...
	{
		Long number = (Long)integerFormat.parseObject( _string, _pos );

		if (number == null)
		{
			throw new ParseException(getErrorMessage( _string ), _pos.getIndex());
		}
		...
		if (found == null)
		{ //TODO this should probably be moved to finder
		...
		Integer type = (Integer)DependencyType.Format.getInstance().parseObject( _string, _pos );
		...
		Duration duration;
		String durationPart = _string.substring( _pos.getIndex() ).trim();

		if (durationPart.length() == 0)
		{ // if a duration was entered, use it, otherwise 0
			duration = Duration.ZERO;
		}
		else
		{
			duration = (Duration)DurationFormat.getInstance().parseObject( _string, _pos );
		...
		return Dependency.getInstance( parameters.isLeftAssociation()
			? (HasDependencies)found
			: (HasDependencies)parameters.getThisObject(),
			parameters.isLeftAssociation()
			? (HasDependencies)parameters.getThisObject()
			: (HasDependencies)found, type.intValue(), duration.getEncodedMillis() );
	}
