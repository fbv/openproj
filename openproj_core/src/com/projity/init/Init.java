...
{
	private Init(
		final boolean _loadConfiguration) 
	{
		...
		if (Environment.getStandAlone() == false)
		{
			try 
			{
				Class.forName( "org.codehaus.groovy.control.ResolveVisitor" ).getMethod( "setClassResolverFilter", 
					new Class[]
				{
					ClassResolverFilter.class
				}).invoke( null, 
					new Object[]
				{
					ScriptConfiguration.getInstance()
				});
			} 
			catch (Exception e) 
			{
		...
		final boolean _loadConfiguration ) 
	{
		if (instance == null) 
		{
			instance = new Init( _loadConfiguration );
		}
	}
