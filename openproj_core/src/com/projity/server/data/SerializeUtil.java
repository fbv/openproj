...
public class SerializeUtil {
    ...
    public static SerializedDataObject serialize(
		DataObject data, 
		...
		throws IOException
	{
        ...
        if (ZIP
			&&((data instanceof WorkCalendar)
			||(data instanceof EnterpriseResource)
			||(data instanceof ResourceImpl)))
		{ 
			//other are too small to be zipped
    ...
    public static DataObject deserialize(
		final SerializedDataObject _sdata,
		final Session _session ) 
		throws IOException,
			   ClassNotFoundException
	{
    	final ByteArrayInputStream bin = new ByteArrayInputStream( _sdata.getSerialized() );
        ObjectInputStream in;
		
		if ((ZIP == true)
		 && ((_sdata.getType() == DataObjectConstants.CALENDAR_TYPE)
		  || (_sdata.getType() == DataObjectConstants.ENTERPRISE_RESOURCE_TYPE)
		  || (_sdata.getType()== DataObjectConstants.RESOURCE_TYPE)))
		{
...
//		boolean forward = fields.get( "forward", true );
		
		final DataObject data = (DataObject)in.readObject();
        data.setUniqueId( (_session == null)
			? _sdata.getUniqueId()
			: _session.getId() );
        data.setName( _sdata.getName() );
        return data;
    }
}
