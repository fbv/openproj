...
public class Main
{
		...
		final String[] _args )
	{
		...
		{
			ArrayList<String> nonEmptyArgs = new ArrayList<String>(_args.length);

			for (int i = 0; i < _args.length; i++)
			{
				if ((_args[ i ] != null) && (_args[ i ].length() > 0))
				{
					nonEmptyArgs.add( _args[ i ] );
				}
			}
			...
			if (nonEmptyArgs.size() > 0)
			{
				...
				for (Iterator<String> i = nonEmptyArgs.iterator(); i.hasNext() == true;)
				{
					String s1 = i.next();
...
//		com.projity.pm.graphic.gantt.Main.main( formatedArgs );
		System.setProperty( "com.apple.mrj.application.apple.menu.about.name", Environment.getStandAlone()
			? "OpenProj"
			: "Project-ON-Demand" );
		...
		HashMap opts = ApplicationStartupFactory.extractOpts( formatedArgs );
		String osName = System.getProperty( "os.name" ).toLowerCase();

		if (osName.startsWith( "linux" ) == true)
		{
			final String javaExec = ConfigurationFile.getRunProperty( "JAVA_EXE" );
				 ...
				 && (javaExec.length() > 0))
				{
					...
					Messages.getStringWithParam( "Text.javaExecutable",
						new Object[]
						{
							javaExec,
							"JAVA_EXE",
							"$HOME/.openproj/run.conf"
						} ));
				}

				if (opts.containsKey( "silentlyFail" ) == false)
				{
			...
			if ((javaVendor == null) 
			 || (((javaVendor.startsWith( "Sun" ) == false) 
			   && (javaVendor.startsWith( "Oracle" ) == false)) 
			  || (javaVendor.startsWith( "IBM" ) == true)))
			{
...
//		}
		...
		final ApplicationStartupFactory startupFactory = new ApplicationStartupFactory( opts ); 
		StartupFactory.setInstance( startupFactory );

		final MainFrame frame = MainFrame.getInstance( Messages.getContextString( "Text.ApplicationTitle" ), null, null );
		final boolean doWelcome = true; // to do see if project param exists in args
